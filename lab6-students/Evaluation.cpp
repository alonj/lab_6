...
double Evaluation::crossValidation(std::vector<Point>& data, size_t folds)
{
    ...
    for (size_t id=0; id<folds; id++)
    {
        ...
        if (!_knn.train(training))
            return 0;
        test(testing);
        double tempAccuracy = EvaluationMeasures::accuracy(testing);
//        cout<<"fold="<<id+1<<", accuracy="<<tempAccuracy<<endl; // printing out folds for question 3
//        cout<<id+1<<","<<tempAccuracy<<endl; // printing out folds for question 3 - for csv file
        totalAccuracy += tempAccuracy;
    }
